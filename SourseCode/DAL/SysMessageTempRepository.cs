...
namespace CRM.DAL
{
    ...
    /// 短信模板
    /// </summary>
    public partial class SysMessageTempRepository : BaseRepository<SysMessageTemp>, IDisposable
    {
        /// <summary>
        /// 查询的数据
        /// </summary>
        ...
        /// <returns></returns>      
        public IQueryable<SysMessageTemp> DaoChuData(DB_CRMEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<SysMessageTemp>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取短信模板---查看详细，首次编辑
        /// </summary>
        ...
        /// <param name="id">主键</param>
        /// <returns>短信模板</returns>
        public SysMessageTemp GetById(DB_CRMEntities db, string id)
        { 
            return db.SysMessageTemp.SingleOrDefault(s => s.Id == id);
        ...
        /// <param name="id">一条短信模板的主键</param>
        public void Delete(DB_CRMEntities db, string id)
        {
            SysMessageTemp deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.SysMessageTemp.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<SysMessageTemp> collection = from f in db.SysMessageTemp
