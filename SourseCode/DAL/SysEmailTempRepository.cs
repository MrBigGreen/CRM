...
namespace CRM.DAL
{
    ...
    /// 邮件模板
    /// </summary>
    public partial class SysEmailTempRepository : BaseRepository<SysEmailTemp>, IDisposable
    {
        /// <summary>
        /// 查询的数据
        /// </summary>
        ...
        /// <returns></returns>      
        public IQueryable<SysEmailTemp> DaoChuData(DB_CRMEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<SysEmailTemp>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取邮件模板---查看详细，首次编辑
        /// </summary>
        ...
        /// <param name="id">主键</param>
        /// <returns>邮件模板</returns>
        public SysEmailTemp GetById(DB_CRMEntities db, string id)
        { 
            return db.SysEmailTemp.SingleOrDefault(s => s.Id == id);
        ...
        /// <param name="id">一条邮件模板的主键</param>
        public void Delete(DB_CRMEntities db, string id)
        {
            SysEmailTemp deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.SysEmailTemp.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<SysEmailTemp> collection = from f in db.SysEmailTemp
