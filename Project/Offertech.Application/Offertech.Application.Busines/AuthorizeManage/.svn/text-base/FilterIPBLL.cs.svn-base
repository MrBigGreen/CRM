...
namespace Offertech.Application.Busines.AuthorizeManage
{
    ...
    public class FilterIPBLL
    {
        ...
        {
            //缓存key
            string black_cacheKey = "blackIPList_" + OperatorProvider.Provider.Current().LoginInfo.UserId;
            //缓存key
            string white_cacheKey = "whiteIPList_" + OperatorProvider.Provider.Current().LoginInfo.UserId;
            //取得用户对象关系Id
            string objectId = OperatorProvider.Provider.Current().LoginInfo.ObjectId;
            //取得当前角色的黑名单IP段
        ...
        private bool CheckArea(IEnumerable<FilterIPEntity> ipList)
        {
            foreach (var item in ipList)
            {
                ...
                //当前IP
                string strIpAddress = OperatorProvider.Provider.Current().LoginInfo.IPAddress;
