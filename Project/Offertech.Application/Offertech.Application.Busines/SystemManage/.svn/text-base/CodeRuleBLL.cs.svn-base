...
namespace Offertech.Application.Busines.SystemManage
{
    ...
    public class CodeRuleBLL
    {
        ...
        public void SaveForm(string keyValue, CodeRuleEntity codeRuleEntity)
        {
            ...
            {
                //调用单据编码示例
                //codeRuleEntity.Description = service.GetBillCode(OperatorProvider.Provider.Current().LoginInfo.UserId, "", "001");
                service.SaveForm(keyValue, codeRuleEntity);
                //service.UseRuleSeed(OperatorProvider.Provider.Current().LoginInfo.UserId, "", "001");
            }
            catch (Exception)
            {
                throw;
