...
namespace Offertech.Application.Service.BaseManage
{
    ...
    public class UserService : RepositoryFactory<UserEntity>, IUserService
    {
        ...
        {
            var expression = LinqExtensions.True<UserEntity>();
            expression = expression.And(t => t.WeChatId == openId);
            expression = expression.Or(t => t.Mobile == openId);
            return this.BaseRepository().FindEntity(expression);
        }
        #endregion
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(userEntity.RoleId))
                {
                    userRelationEntitys.Add(new UserRelationEntity
                    {
                        ...
                        CreateUserId = OperatorProvider.Provider.Current().LoginInfo.UserId,
                        CreateUserName = OperatorProvider.Provider.Current().LoginInfo.UserName,
                        IsDefault = 1,
                    });
                }
                //岗位
