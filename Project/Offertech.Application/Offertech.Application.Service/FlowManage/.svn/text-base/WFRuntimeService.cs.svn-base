    ...
    public class WFRuntimeService : RepositoryFactory, WFRuntimeIService
    {
        ...
        public bool CreateInstance(Guid processId, string schemeInfoId, WFProcessInstanceEntity wfProcessInstanceEntity, string frmData = null)
        {
            ...
            try
            {
                ...
                processOperationHistoryEntity.Content = "【创建】" + OperatorProvider.Provider.Current().LoginInfo.UserName + "创建了一个流程进程【" + wfProcessInstanceEntity.Code + "/" + wfProcessInstanceEntity.CustomName + "】";
                #endregion
        ...
        public bool NodeVerification(string processId, bool flag, string description = "")
        {
            ...
            try
            {
                ...
                if (wfProcessInstanceEntity.ActivityType == 0)//会签
                {
                    ...
                    foreach (string item in _nodelist)
                    {
                        ...
                        if (_makerList != "-1")
                        {
                            ...
                            {
                                if (OperatorProvider.Provider.Current().LoginInfo.UserId == one || OperatorProvider.Provider.Current().LoginInfo.ObjectId.IndexOf(one) != -1)
                                {
                    ...
                    if (_VerificationNodeId != "")
                    {
                        if (flag)
                        {
                            wfProcessOperationHistoryEntity.Content = "【" + OperatorProvider.Provider.Current().LoginInfo.UserName + "】【" + wfruntime.runtimeModel.nodeDictionary[_VerificationNodeId].name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】同意,备注：" + description;
                        }
                        else
                        {
                            wfProcessOperationHistoryEntity.Content = "【" + OperatorProvider.Provider.Current().LoginInfo.UserName + "】【" + wfruntime.runtimeModel.nodeDictionary[_VerificationNodeId].name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】不同意,备注：" + description;
                        }

                        string _Confluenceres = wfruntime.NodeConfluence(_VerificationNodeId, flag, OperatorProvider.Provider.Current().LoginInfo.UserId, description);
                        var _data = new
                        {
                ...
                else//一般审核
                {
                    if (flag)
                    {
                        wfruntime.MakeTagNode(wfruntime.runtimeModel.currentNodeId, 1, OperatorProvider.Provider.Current().LoginInfo.UserId, description);
                        wfProcessInstanceEntity.PreviousId = wfProcessInstanceEntity.ActivityId;
                        wfProcessInstanceEntity.ActivityId = wfruntime.runtimeModel.nextNodeId;
                        wfProcessInstanceEntity.ActivityType = wfruntime.runtimeModel.nextNodeType;//-1无法运行,0会签开始,1会签结束,2一般节点,4流程运行结束
                        wfProcessInstanceEntity.ActivityName = wfruntime.runtimeModel.nextNode.name;
                        ...
                        wfProcessOperationHistoryEntity.Content = "【" + OperatorProvider.Provider.Current().LoginInfo.UserName + "】【" + wfruntime.runtimeModel.currentNode.name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】同意,备注：" + description;
                    }
                    else
                    {
                        ...
                        wfProcessOperationHistoryEntity.Content = "【" + OperatorProvider.Provider.Current().LoginInfo.UserName + "】【" + wfruntime.runtimeModel.currentNode.name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】不同意,备注：" + description;
                    }
                    var data = new
                    {
                        SchemeContent = wfruntime.runtimeModel.schemeContentJson.ToString(),
        ...
        public bool NodeReject(string processId, string nodeId, string description = "")
        {
            try
            {
                ...
                wfruntime.MakeTagNode(wfruntime.runtimeModel.currentNodeId, 0, OperatorProvider.Provider.Current().LoginInfo.UserId, description);
                ...
                wfProcessSchemeEntity.SchemeContent = data.ToJson().ToString();
                wfProcessOperationHistoryEntity.Content = "【" + OperatorProvider.Provider.Current().LoginInfo.UserName + "】【" + wfruntime.runtimeModel.currentNode.name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】驳回,备注：" + description;
        ...
        private string GetMakerList(dynamic node, string processId)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        UserEntity userEntity = userService.GetEntity(OperatorProvider.Provider.Current().LoginInfo.UserId);
