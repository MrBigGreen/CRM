...
namespace Offertech.Application.Service.FlowManage
{
    ...
    public class WFSchemeInfoService : RepositoryFactory, WFSchemeInfoIService
    {
        ...
        public DataTable GetList(string queryJson)
        {
            try
            {
                string dd =OperatorProvider.Provider.Current().LoginInfo.ObjectId;
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT
	                            distinct w.Id,
	                            w.SchemeCode,
                ...
                {
                    if (OperatorProvider.Provider.Current().LoginInfo.ObjectId != "")
                    {
                        strSql.Append(string.Format(" OR w2.ObjectId in ('{0}','{1}') )", OperatorProvider.Provider.Current().LoginInfo.ObjectId.Replace(",", "','"), OperatorProvider.Provider.Current().LoginInfo.UserId));
                    }
                    else
                    {
                        strSql.Append(" ) ");
