...
namespace Offertech.Application.Service.AuthorizeManage
{
    ...
    public class AuthorizeService : RepositoryFactory, IAuthorizeService
    {
        ...
        public string GetDataAuthorUserId(Operator operators, bool isWrite = false)
        {
            ...
            string userId = operators.LoginInfo.UserId;
        ...
        {
            //如果是系统管理员直接给所有数据权限
            if (operators.LoginInfo.IsSystem)
            {
                return "";
            }
            IRepository db = new RepositoryFactory().BaseRepository();
            ...
            if (isWrite)
            {
                strAuthorData = @"   SELECT * FROM Base_AuthorizeData WHERE IsRead=0 AND ObjectId IN ( SELECT  ObjectId FROM    Base_UserRelation WHERE   UserId =@UserId)";
            }
            else
            {
                strAuthorData = @"   SELECT * FROM Base_AuthorizeData WHERE ObjectId IN ( SELECT  ObjectId FROM    Base_UserRelation WHERE   UserId =@UserId)";
            }
            DbParameter[] parameter =
            {
                DbParameters.CreateDbParameter("@UserId",userId),
            ...
            foreach (AuthorizeDataEntity item in listAuthorizeData)
            {
                switch (item.AuthorizeType)
                {
                    ...
                    //本人及下属
                    case -2://
                        whereSb.AppendFormat("  OR ManagerId ='{0}'", userId);
                        break;
                    case -3:
                        whereSb.AppendFormat(@"  OR DepartmentId = (  SELECT  DepartmentId FROM    Base_User WHERE   UserId ='{0}' )", userId);
                        break;
                    case -4:
                        whereSb.AppendFormat(@"  OR OrganizeId = ( SELECT  OrganizeId FROM    Base_User WHERE   UserId ='{0}' )", userId);
                        break;
                    case -5:
                        whereSb.AppendFormat(@"  OR DepartmentId='{1}' OR OrganizeId='{1}'", userId, item.ResourceId);
                        break;
                }
            }
            whereSb.Append(")");
