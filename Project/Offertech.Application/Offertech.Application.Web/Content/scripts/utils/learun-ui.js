...
$.fn.ComboBox = function (options) {
    ...
    if (options) {
        if ($select.find('.ui-select-text').length == 0) {
            ...
            if (options.allowSearch) {
                $select_html += "<div class=\"ui-select-option-search\"><input type=\"text\" class=\"form-control\" placeholder=\"搜索关键字，按回车查询\" /><span class=\"input-query\" title=\"Search\"><i class=\"fa fa-search\" title=\"按回车查询\"></i></span></div>";
            }
            $select_html += "</div>";
            $select.html('');
    ...
    //操作搜索事件
    if (options.allowSearch) {
        $option.find('.ui-select-option-search').find('input').bind("keypress", function (event) {
...
$.fn.ComboBoxSetValue = function (value) {
    ...
    if ($.isNullOrEmpty(value)) {
        ...
        var first = $option.find('ul').children(':first');
        var data_text = first.html();

        $select.attr('data-text', data_text);
        $select.find('.ui-select-text').html(data_text).css('color', '#000');
        first.addClass('liactive')
        return $select;
    }
    else {
