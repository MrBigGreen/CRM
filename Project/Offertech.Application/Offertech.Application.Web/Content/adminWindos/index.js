    ...
    $.learunindex = {
        ...
        loadMenu: function () {
            ...
            $.each(data, function (i) {
                var row = data[i];
                if (row.F_ParentId == "0") {
                    _html += '<li><a href="#" data-id="' + row.F_ModuleId + '"><i class="fa ' + row.F_Icon + '"></i><span>' + row.F_FullName + '</span></a><em></em></li>';
                }
            });
            ...
            $ul.find("li").click(function () {
                ...
                var childNodes = $.learunindex.jsonWhere(data, function (v) { return v.F_ParentId == dataId });
                var _childNodeshtml = "";
                $.each(childNodes, function (i) {
                    ...
                    if (subchildNodes.length > 0) {
                        _childNodeshtml += '<li>';
                        _childNodeshtml += '<a href="#"><i class="fa ' + row.F_Icon + '"></i><span>' + row.F_FullName + '</span><i class="fa fa-angle-left pull-right"></i></a>';
                        _childNodeshtml += '<ul>';
                        $.each(subchildNodes, function (i) {
                            var subrow = subchildNodes[i];
                            _childNodeshtml += '<li><a class="menuItem" data-id="' + subrow.F_ModuleId + '" href="' + subrow.F_UrlAddress + '"><i class="fa ' + subrow.F_Icon + '"></i><span>' + subrow.F_FullName + '</span></a></li>';
                        });
                        _childNodeshtml += '</ul>';
                        _childNodeshtml += '</li>';

                    } else {
                        _childNodeshtml += '<li><a class="menuItem" data-id="' + row.F_ModuleId + '" href="' + row.F_UrlAddress + '"><i class="fa ' + row.F_Icon + '"></i><span>' + row.F_FullName + '</span></a></li>';
                    }
                });
            ...
            if ($(".slidebox").length > 0) {
                ...
                $.each(data, function (i) {
                    ...
                    if (row.F_Target == "iframe") {
                        ...
                        _html += '<li class="menuItem" data-id="' + row.F_ModuleId + '" href="' + row.F_UrlAddress + '">';
                        _html += '    <div class="icon" style="background: #' + color[colorindex] + ';">';
                        _html += '        <i class="fa ' + row.F_Icon + '"></i>';
                        _html += '     </div>';
                        _html += '     <div class="icon-text">';
                        _html += '         <span>' + row.F_FullName + '</a>';
                        _html += '     </div>';
                        _html += '</li>';
                    }
                });
        ...
        indexOut: function () {
            dialogConfirm("注：您确定要安全退出本次登录吗？", function (r) {
                if (r) {
                    $.learunUIBase.loading({ isShow: true ,text:"正在安全退出..."});
                    window.setTimeout(function () {
                        $.ajax({
        ...
        IndexHref: function (pageStyle) {//页面风格
            window.location.href = contentPath + pageStyle;
        }
    };
})(jQuery);