...
namespace Offertech.Application.Web.Areas.BaseManage.Controllers
{
    ...
    public class UserController : MvcControllerBase
    {
        ...
        /// <returns>返回机构+部门+用户树形Json</returns>
        [HttpGet]
        public ActionResult GetCurrentTreeJson(string keyword)
        {
            string CompanyId = OperatorProvider.Provider.Current().LoginInfo.CompanyId;
            string DepartmentId = OperatorProvider.Provider.Current().LoginInfo.DepartmentId;

            var organizedata = organizeCache.GetChildList(CompanyId);
            var departmentdata = departmentCache.GetChildList(DepartmentId);
            var userdata = userCache.GetList();
            var treeList = new List<TreeEntity>();
