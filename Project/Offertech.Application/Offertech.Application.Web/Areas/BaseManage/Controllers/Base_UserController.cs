...
namespace LeaRun.Application.Web.Areas.BaseManage.Controllers
{
    ...
    /// 描 述：用户信息表
    /// </summary>
    public class Base_UserController : MvcControllerBase
    {
        private Base_UserBLL base_userbll = new Base_UserBLL();
        ...
        {
            var data = base_userbll.GetList(queryJson);
        ...
        {
            var data = base_userbll.GetEntity(keyValue);
            return ToJsonResult(data);
        }
        #endregion
        ...
        {
            base_userbll.RemoveForm(keyValue);
            return Success("删除成功。");
        }
        /// <summary>
        /// 保存表单（新增、修改）
