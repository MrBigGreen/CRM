...
namespace Offertech.Util.WebControl
{
    /// <summary>
    /// 树形结构查询
    /// </summary>
    ...
    {        
        ...
        public static List<T> TreeWhere<T>(this List<T> entityList, Predicate<T> condition, string primaryKey, string parentId = "ParentId") where T : class
        {
            ...
            foreach (T entity in locateList)
            {
                ...
                treeList.Add(entity);
                //向上查询
                var pId = entity.GetType().GetProperty(parentId).GetValue(entity, null);
                while (true)
                {
                    if (string.IsNullOrEmpty(pId.ToString()) && pId.ToString() == "0")
                    {
                        break;
                    }
                    ...
                    if (upRecord != null)
                    {
                        ...
                        pId = upRecord.GetType().GetProperty(parentId).GetValue(upRecord, null);
                    }
                    else
                    {
                        break;
        ...
        /// <param name="parentId"></param>
        /// <returns></returns>
        public static List<T> TreeWhere2<T>(this List<T> entityList, Predicate<T> condition, string primaryKey, string parentId = "ParentId") where T : class
        {
            List<T> locateList = entityList.FindAll(condition);
            var parameter = Expression.Parameter(typeof(T), "t");
            //模糊查询表达式
