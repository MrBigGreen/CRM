...
{
    public class WxReturnCodeMessage
    {
        private static readonly Dictionary<int, string> DicMsg;
         static WxReturnCodeMessage()
        {
             DicMsg = new Dictionary<int, string>
             {
                 {-1, "系统繁忙"},
                 {0, "请求成功"},
        ...
        {
            var result=  DicMsg.Where(m=>m.Key==returnCode).ToList();
            if (result.Any())
            {
