...
namespace Offertech.Util.FlowWork
{
    ...
    public class WF_Runtime : IWF_Runtime
    {
        ...
        /// <param name="currentNodeId">当前节点</param>
        /// <param name="frmData">表单数据</param>
        public WF_Runtime(WF_RuntimeInitModel wfRuntimeInitModel, WF_SalaryModel WFSalaryEntity, GetFrmData getFrmData = null)
        {
            _runtimeModel = new WF_RuntimeModel();
            _getFrmData = getFrmData;
            dynamic schemeContentJson = wfRuntimeInitModel.schemeContent.ToJson();//获取工作流模板内容的json对象;
        ...
        /// <param name="paramValue"></param>
        /// <returns></returns>
        private bool LineCompared_String(string frmvalue, string operation, string paramValue)
        {
            bool res = false;
            switch (operation)
            {
        ...
        private string SqlBuider(dynamic schemeContentJson, string frmData, string keyValue)
        {
            try
            {
                if (schemeContentJson.Frm.isSystemTable.Value == 1)
                {
                    ...
                    list.Add(schemeContentJson.Frm.FrmTableId.Value);
                    ...
                    foreach (var item in LineList)//轮训该节点所有连接的线路
                    {
                        ...
                        foreach (var _item in item.setInfo.ConditionValueJson)//轮询该线条上的所有条件
                        {
                            if (!string.IsNullOrEmpty(frmDataJson[_item.ParamName.Value.ToLower()].Value))
                            {
                                ...
                                decimal _frmvalue = 0;
                                decimal _paramValue = 0;
                                if (decimal.TryParse(frmvalue, out _frmvalue) && decimal.TryParse(paramValue, out _paramValue))
                                {
                                    compareValue = LineCompared(frmvalue, operation, paramValue);
                                }
                                else
                                {
                                    compareValue = LineCompared_String(frmvalue, operation, paramValue);
                                }
        ...
        {
            if (!_runtimeModel.nodeDictionary.ContainsKey(nodeId))
            {
                throw (new Exception("寻找不到下一个节点"));
            }
